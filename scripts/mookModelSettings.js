...
function settingIndexToString(str1_, str2_) {
  return game.settings.settings.get(str1_).choices[
    game.settings.get("mookAI", str2_)
  ];
}
...
export class MookModelSettings {
  constructor(token_) {
    ...
    this.mookInitiative =
      MookInitiative[
        settingIndexToString("mookAI.MookInitiative", "MookInitiative")
      ];

    if (
      this.mookInitiative === MookInitiative.ROTATE &&
      this.rotationCost === 0
    )
    ...
    this.standardMeleWeaponTileRange = game.settings.get(
      "mookAI",
      "StandardMeleTileRange"
    );
    if (this.standardMeleWeaponTileRange < 0)
      this.standardMeleWeaponTileRange = 1;
    this.standardRangedWeaponTileRange = game.settings.get(
      "mookAI",
      "StandardRangedTileRange"
    );
