...
function getDistanceMetric() {
  return MinkowskiParameter[
    game.settings.settings.get("mookAI.DistanceMetric").choices[
      game.settings.get("mookAI", "DistanceMetric")
    ]
  ];
}
...
export class MookAI {
  ...
  ready() {
    ...
    document.addEventListener("keyup", (evt) => {
      if (
        evt.key !== "b" ||
        !evt.target.classList.contains("game") ||
        this.busy
      )
        return;
    ...
    if (game.modules.get("lib-find-the-path")?.active) {
      document.addEventListener("keyup", (evt) => {
        if (
          evt.key.toLowerCase() !== "g" ||
          !evt.target.classList.contains("game") ||
          this.busy
        )
          return;

        if (evt.shiftKey) this.takeControlledTurns();
        else if (evt.ctrlKey) this.takeNextTurn();
        else if (evt.altKey) this.takeNextTurn();
        else this.takeNextTurn();
      });
    } else {
      const str =
        "mookAI | Missing module dependency: Library - Path Planning. Please check that it is installed and enabled. mookAI cannot automate without it.";
  ...
  async takeNextTurn() {
    ...
    if (this._combats.size === 0) await this.startCombats();

    let success = await this.takeMookTurn(
      this.getMook(this.getCombat(), game.combat.current.tokenId)
    );
    ...
    try {
      if (!mook_) {
        ui.notifications.warn(
          "mookAI | Mook not found in scene. Please verify that the current scene is active."
        );
        throw (
          "Failed to find mook (id: " +
          game.combat.current.tokenId +
          ") in scene (id: " +
          game.scenes.active.id +
          "). The most likely cause is that you are viewing an inactive scene. Please activate the scene before using mookAI. If the scene is already active, please submit a bug report!"
        );
      }
