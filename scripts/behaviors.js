...
export function getMookType(index_) {
  if (index_ === 0) return MookTypes.EAGER_BEAVER;
...
export class Behaviors {
  ...
  static attackByValue(mm_, targets_, evaluator_, maximize_) {
    ...
    const meleToken = comparator(
      targets_.mele?.map((t) => t.token),
      evaluator_
    );
    const rangedToken = comparator(
      targets_.ranged?.map((t) => t.token),
      evaluator_
    );

    if (!meleToken && !rangedToken) return null;
  ...
  static attackByCurrentHealth(mook_, targets_, gmTech_) {
    ...
    return Behaviors.attackByValue(
      mook_.mookModel,
      targets_,
      getHealth,
      gmTech_
  ...
  static surprise(mook_, targets_) {
    const mm = mook_.mookModel;
    const numTargets =
      (mm.hasMele ? targets_.mele.length : 0) +
