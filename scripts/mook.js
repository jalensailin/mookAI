...
import {
  PointFactory,
  SquareNeighborAngles,
  AngleTypes,
} from "../../lib-find-the-path/scripts/point.js";
...
export class Mook {
  constructor(token_, metric_) {
    ...
    this.pcWarning =
      '<p style="color:red">Warning: Token is owned by a player!</p>';
    this.debug = false;
  }
  ...
  async sense() {
    ...
    this._visibleTargets = game.combat.combatants
      .filter((combatant) => {
      ...
      switch (action.actionType) {
        ...
        case ActionType.EXPLORE:
          if (this.isExploreDisabled)
            this.handleFailure(
              new Abort(
                "Not taking turn. Mook found no targets and exploration is disabled."
              )
            );
          ...
          if (!this._isExplorer) {
            let dialogContent =
              "<p>The mook could not find a target. This could be because they don't have vision on a PC or because they are outside of weapon range.</p><p>The mook can explore their environment and try to find a target. Otherwise, mookAI will return control to the user.</p>";
            ...
            try {
              ...
              this.handleFailure(
                new Abort("Mook not exploring; out of actions.")
              );
            }
        ...
        case ActionType.TRAVERSE:
          ...
          if (action.cost > 0) {
            ...
            this.utility.highlightPoints(
              action.data.path.path.map((s) => s.origin)
            );
          }
  ...
  handleTokenUpdate(changes_) {
    if (changes_._id !== this.token.id) return;
  ...
  get viableTargets() {
    ...
    if (meleTargets.length === 0 && rangedTargets.length === 0) return null;
  ...
  async move(segment_) {
    ...
    await this.rotate(
      this.segment.radialDistToSegment(
        segment_,
        this.token.data.rotation,
        AngleTypes.DEG
      )
    );
    await this.tokenDoc
      .update({ x: segment_.point.px, y: segment_.point.py })
      .catch((err) => {
  ...
  async step() {
    ...
    for (let angle of angles) {
      let success = await this.move(
        this.segment.neighbor(angle, this.rotation)
      );
  ...
  isTargetReachable(target_, attackRange_) {
    return (
      this.pathManager.path(this.token.id, target_.id).terminalDistanceToDest <=
      attackRange_
    );
  }
  ...
  get pathManagerConfig() {
    this._pathManagerConfig.constrainVision = !game.settings.get(
      "mookAI",
      "MookOmniscience"
    );
  ...
  get rotation() {
    return this.token.data.rotation;
  }
